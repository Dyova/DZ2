...
void drawDirected (Graph_t* graph, int size) {
    FILE* S = fopen("dirGraph.dot", "w");                   //write in .dot file
    int checkUncon = 0;
    ...
    system("dot -Tpng dirGraph.dot -o dirGraph.png");       //call system for png
}
...
void drawUndirected(Graph_t* graph, int size) {
    ...
    FILE* S = fopen("undirGraph.dot", "w");                 //write in .dot file
    int checkUncon = 0;
    fprintf(S, "graph Hey {\n");                        
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
    ...
    system("dot -Tpng undirGraph.dot -o undirGraph.png");  //call system for png
}
